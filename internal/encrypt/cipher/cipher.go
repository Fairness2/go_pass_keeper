...
// Config представляет структуру конфигурации, используемую для генерации криптографического ключа.
// Ключ — это начальное значение ключа, предоставленное для вывода.
// Соль — это случайное значение, используемое для обеспечения уникального получения ключа.
// Итерации — это количество итераций, примененных в процессе получения ключа.
// Длина определяет желаемую длину производного ключа в байтах.
type Config struct {
	Key        []byte
	Salt       []byte
	Iterations int
...
func padKey(pass, salt []byte, iterations, length int) []byte {
